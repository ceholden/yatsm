...
For these two circumstances, the :ref:`yatsm_map.py <yatsm_map>` script provides the ``--before`` and ``--after`` flags. These flags are not mutually exclusive and will not override the default behavior of providing the timeseries segment information which intersects the date provided. Instead, these three choices operate in order of desirability. From most to least desirable:
...
1. Create a map showing the image predicted for January 1st, 2000 for all bands::
    ...
    yatsm_map.py predict 2000-01-01 predict_2000-01-01.gtif
